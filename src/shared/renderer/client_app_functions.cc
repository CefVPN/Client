...
namespace client
{
    ...
    else if (name == "importProfile")
    {
      if(arguments.size() == 3 && arguments[2]->IsFunction() && !arguments[1]->IsNull()) {
        ...
        if(!arguments[1]->GetStringValue().empty()) {
          callback_map_.insert(
            std::make_pair(
              std::make_pair(arguments[0]->GetStringValue(), browserID),
              std::make_pair(v8_context, arguments[2])
            )
          );
  ...
  void Cefvpn_v8Handler::ExecuteFunction(
    ...
    CefRefPtr<CefListValue> argsList,
    int value
  )
  {
    if (name == "CEFVPN:STATE:CONNECTED")
    {
      ...
      if(!callback_map_.empty()) {
        ...
        callback_map_.find(
          std::make_pair(name, browser->GetMainFrame()->GetIdentifier())
        );
        ...
        if(it != callback_map_.end()) {
          ...
          CefV8ValueList args;
          args.push_back(CefV8Value::CreateString(argsList->GetString(0)));
